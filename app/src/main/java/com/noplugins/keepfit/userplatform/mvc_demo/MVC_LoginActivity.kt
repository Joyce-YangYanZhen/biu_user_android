...
import com.noplugins.keepfit.userplatform.activity.PasswordSettingActivity
...
class MVC_LoginActivity : BaseActivity() {
    var login_activity_model = LoginModel()
                ...
                params, ProgressSubscriber("验证验证码", object : SubscriberOnNextListener<Bean<LoginBean>> {
                    override fun onNext(code: Bean<LoginBean>) {
                        ...
                        if (code.data.firstLoad == 1) {
                            val intent = Intent(this@MVC_LoginActivity, PasswordSettingActivity::class.java)
                            startActivity(intent)
                            finish()
                        } else {
                            val intent = Intent(
                                this@MVC_LoginActivity,
    ...
    private fun passwordLogin() {
        ...
        login_activity_model.PasswordLoginModel(this, password_login_params, object : Login_Password_CallBack {
            override fun Success(result: Bean<LoginBean>) {
                SpUtils.putString(applicationContext, AppConstants.PHONE, edit_phone_number.text.toString())
